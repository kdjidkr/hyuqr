export default async function handler(req, res) {
    ...
    if (!seatId) {
        return res.status(400).json({ message: 'Missing seatId' });
    }

    try {
        const fetchRes = await fetch('https://lib.hanyang.ac.kr/pyxis-api/2/api/seat-charges', {
            method: 'POST',
            headers: {
        ...
        console.error('Seat Reservation API error:', err);
